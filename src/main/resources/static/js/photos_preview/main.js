...
function begin(){
    var html=document.getElementById("photos").innerHTML;
    var work_id=localStorage.getItem("projectdetail_id");
    var pics;
    $.ajax({
        type : 'POST',
        url : '/project/read',
    ...
    for(var i=0;i<pics.length;i++){
        urls.push("http://"+pics[i]);
    }

    for(var i=0;i<urls.length;i++){
        html+='<div class="grid__item">' +
            '<a class="grid__link" href="#"><img class="grid__img" src="'+urls[i]+'" alt="Some image" /></a>' +
            '</div>'
    }
