...
public final class MeanHash {
    ...
    public MeanHash(byte[] hashValue) {
    ...
    public MeanHash(String hashValue) {
        this(toBytes(hashValue));
    }
    public MeanHash(BufferedImage src){
        this(hashValue(src));
    }
    ...
    /**
     * 从压缩格式指纹创建{@link MeanHash}对象
     * @param compactValue
     * @return
     */
    public static MeanHash createFromCompact(byte[] compactValue){
        return new MeanHash(uncompact(compactValue));
    }
    ...
    public boolean equals(Object obj) {
        if(obj instanceof MeanHash){
            return Arrays.equals(this.binaryzationMatrix,((MeanHash)obj).binaryzationMatrix);
        }else
    ...
    public float compare(String hashValue){
        return compare(new MeanHash(hashValue));
    }
    /**
     * 与指定的指纹比较相似度
     * @param hashValue
    ...
    public float compare(BufferedImage image2){
        return compare(new MeanHash(image2));
    }
    /**
     * 比较指纹相似度
     * @param src
     ...
     */
    public float compare(MeanHash src){
    ...
    private static float compare(byte[] f1,byte[] f2){
        if(f1.length!=f2.length)
            throw new IllegalArgumentException("mismatch MeanHash length");
        int sameCount=0;
    ...
    public static float compare(BufferedImage image1,BufferedImage image2){
        return new MeanHash(image1).compare(new MeanHash(image2));
    }
}